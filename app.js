...
async function loadStatus() {
  ...
  if (!data.configured) {
    notify('.env 설정이 필요합니다. .env.example을 복사해 KIS 앱키/시크리트/계좌번호를 입력하세요.', 'warn');
  }
}
...
function renderSummary(summary) {
  ...
  $('#summaryCards').innerHTML = items.map(([label, value]) => `
    <article class="card"><span>${label}</span><strong>${number(value)} 원</strong></article>
  `).join('');
}
...
function renderBalance(page) {
  ...
  if (!rows.length) emptyRow(tbody, 8, '표시할 보유 종목이 없습니다.');
    ...
    tbody.innerHTML = rows.map(row => `
      <tr>
        <td><strong>${row.prdt_name || row.pdno || '-'}</strong><small>${row.pdno || ''}</small></td>
        <td>${number(row.hldg_qty)}</td>
        <td>${number(row.ord_psbl_qty)}</td>
        <td>${number(row.pchs_avg_pric, 2)}</td>
        <td>${number(row.prpr)}</td>
        <td>${number(row.evlu_amt)}</td>
        <td class="${Number(row.evlu_pfls_amt) >= 0 ? 'up' : 'down'}">${signed(row.evlu_pfls_amt)}</td>
        <td class="${Number(row.evlu_pfls_rt) >= 0 ? 'up' : 'down'}">${signed(row.evlu_pfls_rt, '%')}</td>
      </tr>
    `).join('');
  }
    ...
    tbody.innerHTML = rows.map(row => {
      ...
      return `
        <tr>
          ...
          <td><strong>${row.prdt_name || row.pdno || '-'}</strong><small>${row.pdno || ''}</small></td>
          <td>${sideName}</td>
          <td>${number(row.ord_qty)}</td>
          <td>${number(row.ord_unpr)}</td>
          <td>${number(row.tot_ccld_qty)}</td>
          <td>${number(row.avg_prvs, 2)}</td>
          <td>${row.odno || '-'}</td>
        </tr>`;
    }).join('');
  }
...
$('#ordersForm').addEventListener('submit', event => {
  ...
  state.orders = {
    ...
    pages: [],
    filters: {
      startDate: yyyymmdd($('#ordersStart').value),
      endDate: yyyymmdd($('#ordersEnd').value),
      side: $('#ordersSide').value,
      status: $('#ordersStatus').value,
      productCode: $('#ordersCode').value.trim()
    }
  };
...
$('#tradeForm').addEventListener('submit', async event => {
  ...
  try {
    ...
    const result = await api('/api/order', {
      method: 'POST',
      body: JSON.stringify({ side, productCode, quantity, price, orderType, exchange: 'KRX' })
    });
